...
#usage
#runTest_VBR  ${InputYUV}  ${OutputFile}   ${TargetBR} ${MaxKeyFrameD}  ${CPUUsed}  ${LogFile} 
runTest_VBR()
{
	...
	then
		echo "not enough parameters!"
		echo "usage: runTest_VBR \${InputYUV} \${OutputFile}  \${TargetBR} \${MaxKeyFrameD} \${CPUUsed} \${LogFile} "
		return 1
	fi
	echo ""
...
runMain_JoinTest()
{
	...
	local AllPerformFile="JoinTest-vp9-openh264.csv"
	declare -a aTargetBitRate 
	declare -a aTestYUVSet
	declare -a aCPUUsed
	declare -a aVP9PerForInfo
	...
	do
		...
		do
			...
			do
				...
				LogFile="${YUV}_Target_${TargetBitRate}_CPU_${CPUUsed}_vp9.log"
				echo ""
				echo "target bitrate is ${TargetBitRate}"
				
				runTest_VBR  "${TestSetPath}/${YUV}"  ${OutputFile}  ${TargetBitRate} ${MaxKeyFrameD} ${CPUUsed} ${LogFile}

				echo ""
				cat ${LogFile}
				echo ""
				aVP9PerForInfo[$i]=`runGetPerformanceInfo_VP9   ${LogFile}`
 								
			done
			echo ""
			echo "VP9 Test end in this loop...."
			echo ""
			...
			echo "${YUV}, ${TargetBitRate}, ,${aVP9PerForInfo[0]}, ,${aVP9PerForInfo[1]}, ,${aVP9PerForInfo[2]}, ,${Openh264PerforInfo}">>${AllPerformFile}
		done	
